...
export default function NavBarLg({
  ...
  const handleLogout = () => {
    // remove auth cookies (use the same keys you set earlier)
    Cookies.remove("token");
    Cookies.remove("userEmail");
    Cookies.remove("userRole"); // correct cookie key

    // remove only auth-related localStorage keys (avoid clearing everything)
    ...
    // close mobile menu if open and navigate
            ...
            item.text === "Logout" ? (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon
                    size={25}
                    className="text-teal dark:text-light-teal"
                  />
