...
const PatientDashboard = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (openDropdown && !event.target.closest('.stat-box') && !event.target.closest('.dropdown-menu')) {
  ...
  const handleCancel = async (appointment) => {
    const patientId = 1;
    const success = await cancelAppointment(patientId, appointment.appointmentId);
  ...
  const handleReschedule = async (appointment) => {
    ...
    const success = await rescheduleAppointment(appointment.appointmentId, newDate, newTime);
    ...
    <div className="dashboard bg-color">
      <div className="header">
        <div>
          <h1 className="greeting">
            {greeting}, <span className="patient-highlight">{patientProfile?.fullName || "Ahmad"}!</span>
          </h1>
        ...
        {showAppointments && (
          <>
            <div className="appointments-header">
              ...
              <button className="book-new bg-teal" onClick={handleBookAppointment}>
                ➕ Book Now
              </button>
            </div>
            ...
            {appointments.length === 0 ? (
              <div className="no-appointments">
                <p>No upcoming appointments. Book your first appointment today!</p>
              </div>
            ) : (
                  ...
                  <div className="appt-info">
                    ...
                    <div>
                      <p 
                        ...
                        onClick={() => navigate(`/doctor-profile/${apt.doctorId || apt.id}`)}
                      >
                        {apt.doctorName}
                      </p>
                      <p className="doctor-spec">{apt.specialty}</p>
                      <p className="doctor-time">{apt.time}</p>
                      {apt.date && <p className="appointment-date">{new Date(apt.date).toLocaleDateString()}</p>}
                    </div>
                  </div>
      ...
      {modalData && (
        <Modal
          ...
          confirmText={modalData.type === "cancel" ? "Yes, Cancel" : "Yes, Reschedule"}
          onConfirm={() => modalData.type === "cancel" ? handleCancel(modalData.appointment) : handleReschedule(modalData.appointment)}
          onCancel={closeModal}
        />
      )}
    </div>
