...
const DoctorDashboard = () => {
  ...
  const [modal, setModal] = useState({ visible: false, action: "", appointmentId: null, text: "" });
  ...
  const fetchDoctorProfile = useCallback(async (doctorId) => {
    try {
  ...
  const confirmAppointment = useCallback(async (patientId, appointmentId) => {
    try {
      const response = await fetch(`${API_BASE}/appointments/${patientId}/${appointmentId}/confirm`, {
        method: 'PUT',
        headers: getAuthHeaders()
      });
      return response.ok;
  ...
  const declineAppointment = useCallback(async (patientId, appointmentId) => {
    try {
      const response = await fetch(`${API_BASE}/appointments/${patientId}/${appointmentId}/decline`, {
        method: 'PUT',
        headers: getAuthHeaders()
      });
      return response.ok;
  ...
  const cancelAppointment = useCallback(async (patientId, appointmentId) => {
    try {
      const response = await fetch(`${API_BASE}/appointments/${patientId}/${appointmentId}/cancel`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      });
      return response.ok;
  ...
  const rescheduleAppointment = useCallback(async (appointmentData) => {
    try {
  ...
  const fallbackWeeklyAppointments = useMemo(() => [
    { id: 1, patientName: "Ahmad Mansour", patientId: 1, type: "Consultation", time: "09:00 AM", status: "confirmed", day: "Today" },
    { id: 2, patientName: "Rania Sabbagh", patientId: 2, type: "Follow-up", time: "10:30 AM", status: "pending", day: "Today" },
  ], []);

  const fallbackFeedback = useMemo(() => [
    { id: 1, patientName: "Ahmad Mansour", patientId: 1, rating: 5, comment: "Excellent care..." },
  ], []);
  ...
  const stats = useMemo(() => {
    ...
    const todayAppointmentsList = appointments.filter(apt => apt.date === today);
  ...
  }, [appointments, doctorProfile, fallbackPatients, fallbackWeeklyAppointments, fallbackFeedback]);

  // Event handlers
  const handlePatientNavigation = useCallback((patientId, e) => {
  ...
  const prepareAction = useCallback((apptId, action) => {
    ...
    const text = `Are you sure you want to ${action.toLowerCase()} appointment for ${appt.patientName}?`;
  ...
  const confirmModal = useCallback(async () => {
    ...
    switch(modal.action) {
      case "Accept":
        success = await confirmAppointment(appointment.patientId, modal.appointmentId);
        break;
      case "Decline":
        success = await declineAppointment(appointment.patientId, modal.appointmentId);
        break;
      case "Cancel":
        success = await cancelAppointment(appointment.patientId, modal.appointmentId);
        break;
      default:
        break;
    }
    ...
    if (success) {
      setAppointments(prev => {
        if (modal.action === "Accept") {
          return prev.map(a => a.id === modal.appointmentId ? { ...a, status: "confirmed" } : a);
        }
  ...
  }, [modal, appointments, confirmAppointment, declineAppointment, cancelAppointment]);

  const confirmReschedule = useCallback(async () => {
    const appointment = appointments.find(a => a.id === rescheduleModal.appointmentId);
    if (!appointment) return;
    ...
    setRescheduleModal({ visible: false, appointmentId: null, appointmentName: "", newDate: "", newTime: "" });
  ...
  return (
    ...
    <div className="dashboard">
      <div className="dashboard-header">
        <div>
          <p className="greeting">{greeting}, <span className="doctor-highlight">Dr. Layla Khoury!</span></p>
          <p className="motivational">{motivationalByDay}</p>
          <p className="subtle">Cardiology -- Date: {now.toLocaleDateString()}</p>
        </div>
      </div>
      ...
      {modal.visible && (
        <Modal
          ...
          onCancel={() => setModal({ visible: false, action: "", appointmentId: null, text: "" })}
        />
      )}
      ...
      <RescheduleModal
        ...
        onCancel={() => setRescheduleModal({ visible: false, appointmentId: null, appointmentName: "", newDate: "", newTime: "" })}
